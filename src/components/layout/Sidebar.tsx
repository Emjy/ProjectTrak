...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <aside
        className={`
          fixed left-0 top-0 h-screen w-60 bg-[#0f172a] flex flex-col z-30 border-r border-[#1e293b]
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <span className={`transition-colors duration-150 ${active ? "text-white" : "text-slate-500 group-hover:text-slate-300"}`}>
                  {item.icon}
                </span>
                {item.label}
              </Link>
        ...
        <div className="px-5 py-4 border-t border-[#1e293b]">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-slate-200 truncate">Emilien</p>
