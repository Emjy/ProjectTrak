...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    }).where(eq(tasks.id, params.id)).run();

    // Replace assignees
    if (Array.isArray(body.assigneeIds)) {
      db.delete(taskAssignees).where(eq(taskAssignees.taskId, params.id)).run();
      for (const userId of body.assigneeIds as string[]) {
        const aid = `ta_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;
        db.insert(taskAssignees).values({ id: aid, taskId: params.id, userId }).run();
      }
    }
    ...
    const assignees = db.select().from(taskAssignees).where(eq(taskAssignees.taskId, params.id)).all();
