...
import { Task, TaskStatus, TimeEntryUnit } from '@/types';
...
export default function TaskItem({ task, onView, onEdit, onDelete }: TaskItemProps) {
  const { updateTask, users, currentUser, addTimeEntry } = useApp();
  ...
  const handleActualTimeSave = (actualTime: number, actualTimeUnit: typeof task.actualTimeUnit) => {
    ...
    updateTask(task.id, { status: 'done', actualTime, actualTimeUnit });
  };
  ...
  const handleSaveTimeEntry = async (data: { duration: number; unit: TimeEntryUnit; date: string; note?: string }) => {
    setSavingTime(true);
    try {
      await addTimeEntry({ taskId: task.id, projectId: task.projectId, ...data });
      const minutes = timeToMinutes(data.duration, data.unit as Parameters<typeof timeToMinutes>[1]);
      setRecentMinutes((prev) => (prev ?? 0) + minutes);
  ...
  const { status: timeStatus } = hasEstimate && task.actualTime
  ...
  return (
    <div className={`flex flex-col rounded-xl border transition-all duration-150 group ${isDone ? 'bg-slate-50 border-slate-100' : 'bg-white border-slate-100 hover:border-slate-200 hover:shadow-sm'}`}>
      <div className="flex items-start gap-3 p-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {(hasEstimate || recentMinutes) && (
            <div className="flex items-center gap-2 mt-1.5">
              ...
              {recentMinutes && (
                <span className="text-[11px] text-emerald-600 font-medium">+{formatTime(recentMinutes)} saisi</span>
              )}
              {task.actualTime && hasEstimate && (
        ...
        <div className="flex items-center gap-1.5 md:gap-2 flex-shrink-0">
          ...
          {canLogTime && <button
            ...
            className={`p-1 rounded-md transition-colors opacity-0 group-hover:opacity-100 ${showTimeForm ? 'text-indigo-600 bg-indigo-50 opacity-100' : 'text-slate-400 hover:text-indigo-600 hover:bg-indigo-50'}`}
          >
      ...
      {showTimeForm && (
        <div className="px-3 pb-3 border-t border-slate-100 pt-3 bg-slate-50 rounded-b-xl">
          <TimeEntryForm
            taskId={task.id}
